...
public class MySqlTask {
	...
	public synchronized static Connection open(String dbHost, int dbPort, String dbUser, String dbPass, String dbName,
		...
		if (!sql.tableExists(dbPrefix + TableTotal, connection)) {
			sql.modifyQuery(
					"CREATE TABLE `" + dbPrefix + TableTotal
							+ "` (`IGN` varchar(32) NOT NULL, `votes` int(10) DEFAULT 0, `lastvoted` BIGINT(16) DEFAULT 0, PRIMARY KEY (`IGN`));",
					connection);
		} else {
			...
			try {
				...
				sql.modifyQuery("ALTER TABLE `" + dbPrefix + TableTotal
						+ "` ADD  `lastvoted` BIGINT(16) DEFAULT 0 AFTER `votes`;", connection);
			}
		}

		if (AurionsVoteListener.GetInstance().old) {
			sql.modifyQuery("RENAME TABLE `" + dbPrefix + TableQueue + "` TO `" + dbPrefix + TableQueue + "old`;",
					connection);
			sql.modifyQuery(
